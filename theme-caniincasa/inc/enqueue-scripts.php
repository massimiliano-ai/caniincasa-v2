...
function caniincasa_enqueue_page_templates_grid() {
    if ( is_page_template( 'page-templates/template-allevamenti.php' ) ||
        ...
        // Veterinari professional styles
        if ( is_page_template( 'page-templates/template-veterinari.php' ) ) {
            wp_enqueue_style(
                'caniincasa-veterinari',
                CANIINCASA_THEME_URI . '/css/veterinari.css',
                array(),
                CANIINCASA_VERSION
            );
        }
...
/**
 * Enqueue Professional Styles for Single Pages
 */
function caniincasa_enqueue_single_professional_styles() {
    // Apply professional styles to all structure single pages
    if ( is_singular( array( 'struttureveterinarie', 'allevamenti', 'canili', 'centri_cinofili', 'pensioni_per_cani' ) ) ) {
        wp_enqueue_style(
            'caniincasa-veterinari',
            CANIINCASA_THEME_URI . '/css/veterinari.css',
            array(),
...
add_action( 'wp_enqueue_scripts', 'caniincasa_enqueue_single_professional_styles', 20 );
